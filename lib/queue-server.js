...
const promMetrics = {
    connReuseCounter: new promClient.Counter({
        name: 'zonemta_connection_reuses',
        help: 'Connection reuses'
    }),
    connPoolSizeGauge: new promClient.Gauge({
        name: 'zonemta_connection_pool_size',
        ...
        this.server.on('client', client => {
            ...
            client.onData = (data, next) => {
                ...
                switch (data.cmd) {
                    ...
                    case 'COUNTMETRICS':
                        if (data.metric && data.func) {
                            try {
                                if (promMetrics.hasOwnProperty(data.metric)) {
                                    promMetrics[data.metric][data.func]();
                                } else {
                                    log.verbose('QS', 'Invalid prometheus metric used.');
                                }
                            } catch (E) {
                                // And now ?!
                            }
