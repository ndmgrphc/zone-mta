                                ...
                                this.releaseDelivery(delivery, (err, released) => {
                                    ...
                                    plugins.handler.runHooks('sender:delivered', [delivery, info], err => {
                                        if (err) {
                                            log.error(
                                                this.logName,
                                                '%s.%s Failed to process sender:delivered hook. %s',
                                                delivery.id,
                                                delivery.seq,
                                                err.message
                                            );
        ...
        let tryConnect = () => {
            ...
            if (this.connectionPool.has(delivery.connectionKey)) {
                ...
                if (conn) {
                    log.info(
                        this.logName,
                        '%s.%s CONNREUSE [%s] count=%s mx=%s mta=%s',
                        delivery.id,
                        delivery.seq,
                        conn.connection.id,
            ...
            mxOptions.connectError = (err, opts, connOpts) => {
                log.error(
                    this.logName,
                    '%s.%s CONNFAIL domain=%s local=%s[%s] remote=%s:%s error=%s',
                    delivery.id,
                    delivery.seq,
                    opts.domain,
            ...
            mxConnect(mxOptions, (err, mx) => {
                ...
                let _onError = err => {
                    ...
                    if (
                        ...
                        log.info(
                            this.logName,
                            '%s.%s ERRCONNECT [%s] Failed to connect to %s[%s] using STARTTLS, proceeding with plaintext. %s',
                            delivery.id,
                            delivery.seq,
                            connId,
                            mx.hostname,
